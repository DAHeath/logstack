...
memory utilization, as we discuss in~\Cref{sec:ourContrib,
sec:memoryEfficiency}.
...
As discussed in~\HK, an interactive protocol, such as the output selection protocol of~\cite{AC:Kolesnikov18}, may be used for computationally efficient (linear in $b$) garbage collection.  However, a communication round is added for each exit of the conditional.  In many scenarios a non-interactive GC 2PC is preferred, which is the focus of this work.
...
\ignore{\item The conditional branches are \emph{nested}.  That is,
  instead of organizing $b$ branches as a vector, we organize them
  into a binary tree.  Suppose \E guesses that a particular branch
  $\cir_i$ is taken, while in fact $\cir_j$, a member of a different
  subtree, is taken.  The key idea of our approach is that we ensure
  that when \ev garbles the entire subtree containg $\cir_j$ but not
  $\cir_i$, she computes the same material, regardless of
$j$.\todo{intuition still unclear} }
