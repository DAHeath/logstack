...
provide nothing more than infrastructure needed to host our contribution.
%
We briefly discuss details of these less interesting algorithms before returning
...
\begin{figure}
  \begin{align*}
    ...
    \\
    &\halfway(i, j):\\
    &~~\codecomment{Simple helper for range of branches (approximately) in half.}\\
...
\begin{lemma}\label{lemma:gbtreetime}
  If $\Gb$ runs in $O(\nmat)$ space,
  then for all vectors of circuits $\vec \cir$, all $i, j \in \mathbb{N}$
  such that $i \leq j$, and all $seed \in \{0, 1\}^\kappa$,
  $\gbtree(\vec \cir, i, j, seed)$ runs in $O(\nmat \log (j-i+1))$ space.
\end{lemma}
\begin{proof}
...
\begin{figure}
  \begin{align*}
    ...
    subtree.}\\
    &~~~~~~\mat_R, \cdot \gets \gbtree(\vec{\cir}, \sfrac{j}{2}+1, j, es_{\sfrac{j}{2}+1, j})\\
    &~~~~~~\evcond'(\mat \oplus \mat_R, i, \sfrac{j}{2})\\
    &~~~~~~\codecomment{Symmetrically evaluate the right subtree.}\\
    &~~~~~~\mat_L, \cdot \gets \gbtree(\vec{\cir}, i, \sfrac{j}{2}, es_{i,\sfrac{j}{2}})\\
