...
\begin{theorem}
  ...
  property:
  \begin{itemize}
    \item Let $\cir$ be an arbitrary circuit and let $\nmat$ be the size of
      material generated invoking $\underscheme.\gGb$ on $\cir$.
      Then both $\underscheme.\gEv$ and $\underscheme.\gGb$, invoked
      on $\cir$, run in $O(\nmat)$ time and $O(\nmat)$ space.
  \end{itemize}
  Then \Cref{ourconstr} instantiated with $\underscheme$ satisfies the
  following property.
  \begin{itemize}
    \item Let $\vec{\cir}$ be an arbitrary vector of $b$ circuits. Let $\nmat$ be
      the maximum size of the garblings constructed by calling
      $\underscheme.\gGb$ on each of these $b$ branches.
