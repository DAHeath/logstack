...
\begin{figure*}[t]
  ...
  \input{fig/seed-tree}
  \caption{%
    Suppose there are eight branches $\cir_0$ through $\cir_7$, and
...
low: altogether \G\ issues $\frac{3}{2} b\log b + b$ calls to \Gb\ and
$b\ log b$ calls to \Ev.
\E\ issues $b \log b$ calls to \Gb\ and $b$ calls to $\Ev$.

% \G and \E make a total of $\frac{5}{2} b \log b + b$ calls to \Gb\
...
\begin{itemize}
  \item \textsc{Inputs:} the active branch id \aid\ and the number of branches $b$.
  \item \textsc{Outputs:} a sequence of evaluator seeds that form a binary tree:
  \[
...
Recall, the active branch ID \aid\ is available to \G\ in the form of
garbled labels.
...
that we call \gadget.\footnote{%
  ...
  \aid: those that
  are `good' (i.e., sibling roots of \aid) and those that
  are `bad'.
}
...
\aid, \E will obtain a {\em fixed} seed that is predictable to \G. 
...
\subsubsection{$O(b \log b)$ computational cost accounting.}
Let \Gb\ and \Ev\ be procedures that respectively garble/evaluate a
GC.
% We verify the number of calls to \Gb\ and \Ev\ is $O(b \log
% b)$.
Consider how many such calls are
...
the root.  Thus, the total number of calls by \E to \Gb\ is $b \log b$ and
to \Ev\ is exactly $b$.  
...
Now, consider \G's number of calls to \Ev.  Recall that our
...
% Nesting branches may seem desirable.
% Indeed,
...
are uniformly random.  This is sufficient to prove security in the standard model.
